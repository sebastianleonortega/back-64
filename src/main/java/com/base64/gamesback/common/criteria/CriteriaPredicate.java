...
public class CriteriaPredicate<T, X> {
    ...
    public CriteriaQuery<X> convertSearch(CriteriaQuery<X> cq, Root<T> root, Predicate search) {

        this.getJoins(root.getJoins());

        CriteriaPredicateRoot<T> criteriaPredicateRoot = new CriteriaPredicateRoot<>(this.builder);

        List<Predicate> predicates = criteriaPredicateRoot.format(new ArrayList<>(), root);
